...
class BudgetResourceStrategy(ResourceStrategy):
    ...
    async def process_requirement(
        ...
        """
        # Step 1: Retrieve budget resources
        # For MVP, we need to get the tenant_id from somewhere
        # Since BudgetResourceRequirement doesn't have tenant_id, we'll use requester_id
        # In a real implementation, this would come from the request metadata
        budget_resources = await self.retriever.retrieve_budget_candidates(
            tenant_id=requirement.requester_id,  # Use requester's tenant as proxy
