...
from app.agents.agent2_execution.database.models import ProcessInstance, Task
    ...
    if session is not None and proc_id:
        try:
            ...
            if proc_row:
                ...
                ctx.status = proc_row.execution_status or proc_row.status
                if proc_row.requester_email and not ctx.requester_email:
                    ctx.requester_email = proc_row.requester_email
            ...
            if lookup_task_id:
                ...
                if task_row:
                    ...
                    ctx.assigned_to = task_row.assigned_to_email or ctx.assigned_to
