...
No live external ERP is integrated (none exists in this deployment). These
tools persist PO drafts, quotations, and procurement record updates into the
shared processes.metadata_json and workflow_events tables, so Agent 2 has a
real, queryable execution trail in the shared database. When a real ERP is
added, these tools are the seam to replace.
"""
...
from app.agents.agent2_execution.database.persistence import merge_process_metadata, record_workflow_event
