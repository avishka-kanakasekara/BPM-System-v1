"""
Agent 2 — Inter-agent JWT helpers (legacy HS256).

HTTP routes mounted on the monolith use the shared Supabase JWT path
(``app.core.security.get_current_user``). This module remains only for:

- Agent 2's leftover standalone ``/api/v1/messages`` app (unit tests)
- Outbound Agent4Client header signing in those same tests

Production process/approval/agent2 read-governance traffic does not use
this secret. Do not introduce new HTTP dependencies on
