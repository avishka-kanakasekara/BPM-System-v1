"""Legacy HTTP client Agent 2 used when Agent 4 was a separate process.

The monolith talks in-process through Agent 4's Agent2Adapter. This
client is kept only so existing Agent 2 unit tests can still assert
outbound header signing. New code should not call it.
"""
...
from app.agents.agent2_execution.security.auth import create_access_token
